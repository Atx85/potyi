...
fn build(
    ...
    if cfg!(feature = "no-link") || env::var("DOCS_RS").is_ok() {
        ...
        #[cfg(feature = "build-from-source")]
        {
            ...
            // The generated .pc file can contain an unescaped prefix with spaces.
            // Preserve CMake's actual library directories before parsing its flags.
            if !LINK_FRAMEWORK {
                link_flags.search_lib(out_dir.join("lib").display());
                link_flags.search_lib(out_dir.join("lib64").display());
            }

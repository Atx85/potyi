...
`"notes with spaces.txt"`, opens that file in the terminal's own pane when
submitted from its input box. Typed `edit PATH` and `view PATH` commands use
the same pane too. Named documents are saved before replacement; unnamed
documents and failed saves keep their existing protection.
